...
<div class="card">
    <div class="card-header font-weight-bold">
        {{ trans('global.create') }} {{ trans('cruds.biayaSkema.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.biaya-skemas.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('biaya') ? 'is-invalid' : '' }}" name="biaya_id" id="biaya_id">
                    @foreach($biayas as $id => $biaya)
                        <option value="{{ $id }}" {{ old('biaya_id') == $id ? 'selected' : '' }}>{{ $biaya }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('persen_max') ? 'is-invalid' : '' }}" type="number" name="persen_max" id="persen_max" value="{{ old('persen_max') }}" step="0.01" max="100">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('persen_min') ? 'is-invalid' : '' }}" type="number" name="persen_min" id="persen_min" value="{{ old('persen_min') }}" step="0.01" max="100">
