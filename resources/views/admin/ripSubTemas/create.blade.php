        ...
        <div class="col-sm-8">
            <div class="card">
                <div class="card-header font-weight-bold">
                    {{ trans('global.create') }} {{ trans('cruds.ripSubTema.title_singular') }}
                </div>

                <div class="card-body">
                    <form method="POST" action="{{ route("admin.rip-sub-temas.store") }}" enctype="multipart/form-data">
                        @csrf
                        <div class="form-group">
                            ...
                            <select class="form-control select2 {{ $errors->has('tema') ? 'is-invalid' : '' }}" name="tema_id" id="tema_id">
                                @foreach($temas as $id => $tema)
                                    <option value="{{ $id }}" {{ old('tema_id') == $id ? 'selected' : '' }}>{{ $tema }}</option>
                                @endforeach
                            </select>
