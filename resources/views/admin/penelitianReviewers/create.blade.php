        ...
        <div class="col-sm-8">
            <div class="card">
                <div class="card-header font-weight-bold">
                    {{ trans('global.create') }} {{ trans('cruds.penelitianReviewer.title_singular') }}
                </div>

                <div class="card-body">
                    <form method="POST" action="{{ route("admin.penelitian-reviewers.store") }}" enctype="multipart/form-data">
                        @csrf
                        <div class="form-group">
                            ...
                            <select class="form-control select2 {{ $errors->has('tahapan_review') ? 'is-invalid' : '' }}" name="tahapan_review_id" id="tahapan_review_id" required>
                                @foreach($tahapan_reviews as $id => $tahapan_review)
                                    <option value="{{ $id }}" {{ old('tahapan_review_id') == $id ? 'selected' : '' }}>{{ $tahapan_review }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control select2 {{ $errors->has('reviewer') ? 'is-invalid' : '' }}" name="reviewer_id" id="reviewer_id" required>
                                @foreach($reviewers as $id => $reviewer)
                                    <option value="{{ $id }}" {{ old('reviewer_id') == $id ? 'selected' : '' }}>{{ $reviewer }}</option>
                                @endforeach
                            </select>
