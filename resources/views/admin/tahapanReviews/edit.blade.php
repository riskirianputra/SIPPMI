        ...
        <div class="col-sm-8">
            <div class="card">
                <div class="card-header font-weight-bold">
                    {{ trans('global.edit') }} {{ trans('cruds.tahapanReview.title_singular') }}
                </div>
                <form method="POST" action="{{ route("admin.tahapan-reviews.update", [$tahapanReview->id]) }}" enctype="multipart/form-data">
                    <div class="card-body">
                        @method('PUT')
                        @csrf
                        <div class="form-group">
                            <label class="required" for="nama">{{ trans('cruds.tahapanReview.fields.nama') }}</label>
                            <input class="form-control {{ $errors->has('nama') ? 'is-invalid' : '' }}" type="text" name="nama" id="nama" value="{{ old('nama', $tahapanReview->nama) }}" required>
                            @if($errors->has('nama'))
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control {{ $errors->has('jumlah_reviewer') ? 'is-invalid' : '' }}" name="jumlah_reviewer" id="jumlah_reviewer">
                                ...
                                @foreach(App\TahapanReview::JUMLAH_REVIEWER as $key => $label)
                                    <option value="{{ $key }}" {{ old('jumlah_reviewer', $tahapanReview->jumlah_reviewer) === $key ? 'selected' : '' }}>{{ $label }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <input class="form-control {{ $errors->has('tahun') ? 'is-invalid' : '' }}" type="number" name="tahun" id="tahun" value="{{ old('tahun', $tahapanReview->tahun) }}" step="1" required>
                            @if($errors->has('tahun'))
                                <div class="invalid-feedback">
                                    {{ $errors->first('tahun') }}
                                </div>
                        ...
                        <div class="form-group">
                            ...
                            <input class="form-control date {{ $errors->has('mulai') ? 'is-invalid' : '' }}" type="text" name="mulai" id="mulai" value="{{ old('mulai', $tahapanReview->mulai) }}">
                            @if($errors->has('mulai'))
                                <div class="invalid-feedback">
                                    {{ $errors->first('mulai') }}
                                </div>
                        ...
                        <div class="form-group">
                            ...
                            <input class="form-control date {{ $errors->has('selesai') ? 'is-invalid' : '' }}" type="text" name="selesai" id="selesai" value="{{ old('selesai', $tahapanReview->selesai) }}">
