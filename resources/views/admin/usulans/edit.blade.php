...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.usulan.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.usulans.update", [$usulan->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('pengusul') ? 'is-invalid' : '' }}" name="pengusul_id" id="pengusul_id">
                    @foreach($pengusuls as $id => $pengusul)
                        <option value="{{ $id }}" {{ ($usulan->pengusul ? $usulan->pengusul->id : old('pengusul_id')) == $id ? 'selected' : '' }}>{{ $pengusul }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('status_usulan') ? 'is-invalid' : '' }}" name="status_usulan" id="status_usulan">
                    ...
                    @foreach(App\Usulan::STATUS_USULAN_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('status_usulan', $usulan->status_usulan) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('jenis_usulan') ? 'is-invalid' : '' }}" name="jenis_usulan" id="jenis_usulan">
                    ...
                    @foreach(App\Usulan::JENIS_USULAN_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('jenis_usulan', $usulan->jenis_usulan) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
