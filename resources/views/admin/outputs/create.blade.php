        ...
        <div class="col-sm-8">
            <div class="card">
                <div class="card-header font-weight-bold">
                    {{ trans('global.create') }} {{ trans('cruds.output.title_singular') }}
                </div>

                <div class="card-body">
                    <form method="POST" action="{{ route("admin.outputs.store") }}" enctype="multipart/form-data">
                        @csrf
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control select2 {{ $errors->has('jenis_usulan') ? 'is-invalid' : '' }}" name="jenis_usulan_id" id="jenis_usulan_id" required>
                                @foreach($jenis_usulans as $id => $jenis_usulan)
                                    <option value="{{ $id }}" {{ old('jenis_usulan_id') == $id ? 'selected' : '' }}>{{ $jenis_usulan }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <input class="form-control {{ $errors->has('luaran') ? 'is-invalid' : '' }}" type="text" name="luaran" id="luaran" value="{{ old('luaran', '') }}" required>
