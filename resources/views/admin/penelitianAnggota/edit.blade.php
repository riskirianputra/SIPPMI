        ...
        <div class="col-sm-8">
            <div class="card">
                <div class="card-header font-weight-bold">
                    {{ trans('global.edit') }} {{ trans('cruds.penelitianAnggotum.title_singular') }}
                </div>

                <div class="card-body">
                    <form method="POST" action="{{ route("admin.penelitian-anggota.update", [$penelitianAnggotum->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="form-group">
                            ...
                            <select class="form-control select2 {{ $errors->has('dosen') ? 'is-invalid' : '' }}" name="dosen_id" id="dosen_id">
                                @foreach($dosens as $id => $dosen)
                                    <option value="{{ $id }}" {{ ($penelitianAnggotum->dosen ? $penelitianAnggotum->dosen->id : old('dosen_id')) == $id ? 'selected' : '' }}>{{ $dosen }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control select2 {{ $errors->has('penelitian') ? 'is-invalid' : '' }}" name="penelitian_id" id="penelitian_id">
                                @foreach($penelitians as $id => $penelitian)
                                    <option value="{{ $id }}" {{ ($penelitianAnggotum->penelitian ? $penelitianAnggotum->penelitian->id : old('penelitian_id')) == $id ? 'selected' : '' }}>{{ $penelitian }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control {{ $errors->has('jabatan') ? 'is-invalid' : '' }}" name="jabatan" id="jabatan">
                                ...
                                @foreach(App\PenelitianAnggotum::JABATAN_SELECT as $key => $label)
                                    <option value="{{ $key }}" {{ old('jabatan', $penelitianAnggotum->jabatan) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                                @endforeach
                            </select>
