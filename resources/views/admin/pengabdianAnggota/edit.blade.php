        ...
        <div class="col-sm-8">
            <div class="card">
                <div class="card-header font-weight-bold">
                    {{ trans('global.edit') }} {{ trans('cruds.pengabdianAnggotum.title_singular') }}
                </div>

                <div class="card-body">
                    <form method="POST" action="{{ route("admin.pengabdian-anggota.update", [$pengabdianAnggotum->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="form-group">
                            ...
                            <select class="form-control select2 {{ $errors->has('pengabdian') ? 'is-invalid' : '' }}" name="pengabdian_id" id="pengabdian_id">
                                @foreach($pengabdians as $id => $pengabdian)
                                    <option value="{{ $id }}" {{ ($pengabdianAnggotum->pengabdian ? $pengabdianAnggotum->pengabdian->id : old('pengabdian_id')) == $id ? 'selected' : '' }}>{{ $pengabdian }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control select2 {{ $errors->has('dosen') ? 'is-invalid' : '' }}" name="dosen_id" id="dosen_id">
                                @foreach($dosens as $id => $dosen)
                                    <option value="{{ $id }}" {{ ($pengabdianAnggotum->dosen ? $pengabdianAnggotum->dosen->id : old('dosen_id')) == $id ? 'selected' : '' }}>{{ $dosen }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control {{ $errors->has('jabatan') ? 'is-invalid' : '' }}" name="jabatan" id="jabatan">
                                ...
                                @foreach(App\PengabdianAnggotum::JABATAN_SELECT as $key => $label)
                                    <option value="{{ $key }}" {{ old('jabatan', $pengabdianAnggotum->jabatan) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                                @endforeach
                            </select>
