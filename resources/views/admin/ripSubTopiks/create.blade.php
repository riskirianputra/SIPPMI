        ...
        <div class="col-sm-8">
            <div class="card">
                <div class="card-header font-weight-bold">
                    {{ trans('global.create') }} {{ trans('cruds.ripSubTopik.title_singular') }}
                </div>

                <div class="card-body">
                    <form method="POST" action="{{ route("admin.rip-sub-topiks.store") }}" enctype="multipart/form-data">
                        @csrf
                        <div class="form-group">
                            ...
                            <select class="form-control select2 {{ $errors->has('topik') ? 'is-invalid' : '' }}" name="topik_id" id="topik_id">
                                @foreach($topiks as $id => $topik)
                                    <option value="{{ $id }}" {{ old('topik_id') == $id ? 'selected' : '' }}>{{ $topik }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <input class="form-control {{ $errors->has('luaran') ? 'is-invalid' : '' }}" type="text" name="luaran" id="luaran" value="{{ old('luaran', '') }}">
