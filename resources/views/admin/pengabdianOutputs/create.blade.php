        ...
        <div class="col-sm-8">
            <div class="card">
                <div class="card-header font-weight-bold">
                    {{ trans('global.create') }} {{ trans('cruds.pengabdianOutput.title_singular') }}
                </div>

                <div class="card-body">
                    <form method="POST" action="{{ route("admin.pengabdian-outputs.store") }}" enctype="multipart/form-data">
                        @csrf
                        <div class="form-group">
                            ...
                            <select class="form-control select2 {{ $errors->has('output_skema') ? 'is-invalid' : '' }}" name="output_skema_id" id="output_skema_id" required>
                                @foreach($output_skemas as $id => $output_skema)
                                    <option value="{{ $id }}" {{ old('output_skema_id') == $id ? 'selected' : '' }}>{{ $output_skema }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <input class="form-control {{ $errors->has('filename') ? 'is-invalid' : '' }}" type="text" name="filename" id="filename" value="{{ old('filename', '') }}" required>
                        ...
                        <div class="form-group">
                            ...
                            <input class="form-control date {{ $errors->has('tanggal_upload') ? 'is-invalid' : '' }}" type="text" name="tanggal_upload" id="tanggal_upload" value="{{ old('tanggal_upload') }}" required>
