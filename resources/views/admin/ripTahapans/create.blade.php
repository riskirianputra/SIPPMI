        ...
        <div class="col-sm-8">
            <div class="card">
                <div class="card-header font-weight-bold">
                    {{ trans('global.create') }} {{ trans('cruds.ripTahapan.title_singular') }}
                </div>

                <div class="card-body">
                    <form method="POST" action="{{ route("admin.rip-tahapans.store") }}" enctype="multipart/form-data">
                        @csrf
                        <div class="form-group">
                            ...
                            <select class="form-control select2 {{ $errors->has('sub_topik') ? 'is-invalid' : '' }}" name="sub_topik_id" id="sub_topik_id">
                                @foreach($sub_topiks as $id => $sub_topik)
                                    <option value="{{ $id }}" {{ old('sub_topik_id') == $id ? 'selected' : '' }}>{{ $sub_topik }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <textarea class="form-control ckeditor {{ $errors->has('bahasan') ? 'is-invalid' : '' }}" name="bahasan" id="bahasan">{!! old('bahasan') !!}</textarea>
